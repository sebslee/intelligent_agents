...
public class Agent26 extends AbstractNegotiationParty {
	...
	if(acceptOrOffer(lastReceivedOffer, util) ) {
	    ...
	    //40% of the time we generate a "good enough bid", this is to collect enough data to predict the nash offer ...
	    if(getTimeLine().getTime() < 0.7){
		...
		return new Offer (this.getPartyId() , this.generateGoodBid());
	    }
           
           
            if (model_valid(0) && model_valid (1)){
    ...
    // So if there is not a significant difference in each issue max and second max value, then we return true...
	...
	for(int i = 0 ; i < domain_issues.size() ; i ++){
	    ...
	    difference = (min_value * 100 )/ max_value;
	    System.out.format("%d Issue %d max value issue %d min value  %d %f \n", domain_issues.size() , i , max_value, min_value , difference);
	...
	for(Issue lIssue : domain_issues) {
		...
		if(Math.random() > 0.5 +  (additiveUtilitySpace_i.getWeight(lIssue.getNumber())  - min_weight )/ (max_weight - min_weight)) 
		    {
			/*	randr = Math.random();
				if( randr < (0.5)){
	...
	for(int k = 0 ; k < kmax ; k++){
	    ...
	    //	while(random_issue_1 == max_weight_number ){
	    ...
	    curr_energy = our_utility * predicted_a  * predicted_b + a * (1.0/Math.max(0.01, Math.abs(our_utility - predicted_a))) + b * (1.0/Math.max(0.01, Math.abs(our_utility - predicted_b))) ;
	    ...
	    //predicted_b = 1;			
	    next_energy = our_utility * predicted_a  * predicted_b + a * (1.0/Math.max(0.01, Math.abs(our_utility - predicted_a))) + b * (1.0/Math.max(0.01, Math.abs(our_utility - predicted_b)));
	    ...
	    if(this.utilitySpace.getUtility(altered_bid) < 0.9 &&
	       this.utilitySpace.getUtility(altered_bid) > getTargetUtility( 0.1 , 1.5) ){
		// System.out.format("\n In loop %f %f\n", getTargetUtility( 0.1 , 1.5) , this.utilitySpace.getUtility(altered_bid) );
